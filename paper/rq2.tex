...
Of the original 42 pairs, we report scores for only 35 pairs. Due to design flaws, seven pairs were dropped. In one, the regexes evaluated, \verb!\..*! and \verb!\.+!. are not semantically equivalent (the former is missing an escape and should be \verb!\.\.*!). In the other six, the patterns performed transformations from multiple equivalence classes. For example,  \verb!([\072\073])! is in C2 and T4 and  \verb!(:|;)! in C5, T1;  it was not clear if any differences were due to the CCC or LIT groups, or both. However, \verb!([:;])!, could be compared with each, since it is a member of T1 and C2, so comparing it to \verb!([\072\073])! evaluates T1 and T4, and comparing to \verb!(:|;)! evaluates  C2 and C5. In the end, we covered  14 edges from Figure~\ref{fig:refactoringTree}.
