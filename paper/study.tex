...
\noindent {\textbf{RQ1:}} {\em Which regex representations are most smelly based on \emph{understandability} as measured by identifying matching strings and by composing matching strings?}
To answer RQ1, we conduct a study in which programmers are presented with a regex and asked comprehension questions about its matching behavior. By comparing accuracy between regexes that match the same language but are expressed differently (e.g., \verb!tri[a-f]3! and \verb!tri(a|b|c|d|e|f)3!), we can measure understandability. This analysis requires the definition of equivalence classes for regexes. By inspecting  nearly 14,000 regexes extracted from Python projects in a publicly available dataset~\cite{chapman2016}, we formed an initial set of five equivalence classes to explore. \\


\noindent {\textbf{RQ2:}} {\em Which regex representations have the strongest \emph{community support} based on how frequently each representation appears in regexes in open source Python projects?}
To answer RQ2, we explore the publicly available regex dataset~\cite{chapman2016} and use the presence and absence of language features as a proxy for community support, where more frequently-used features are likely to be more understandable.\\
...
Based on the results of RQ1 and RQ2, we observe which regexes are the most desirable based on comprehension metrics and community support. 
