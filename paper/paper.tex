...
\begin{abstract}
Regular expressions (regexes) are powerful tools employed across many tasks and platforms. Regexes can be very complex and prior work has shown that developers find regexes to be difficult to compose and understand. Due to a rich feature set, there is more than one way to compose a regex to get the same desired behavior. 
